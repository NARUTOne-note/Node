...
- `主键索引 PRIMARY`：唯一、非空，InnoDB 聚簇索引，速度最快
- `普通索引 INDEX`：普通查询、关联字段使用
- `唯一索引 UNIQUE`：字段全局唯一（手机号、用户名）
- `联合索引（复合索引）`：多个字段组合索引，遵循最左匹配原则

联合索引 idx_a_b_c(a, b, c)✅` 生效条件：查询条件包含最左前列

**索引失效高频场景**（线上慢 SQL 根源）

- 字段隐式转换：字符串字段用数字匹配 `where username=123`
- 索引列参与运算 / 函数 `where DATE(create_time)='2026-07-06'`
- 模糊查询前置通配符 `username LIKE '%张三'（后缀张三%有效）`
- 使用 `!= <>` is not null
- OR 连接无索引字段

**联表索引规范**

两张表 JOIN 时，ON 关联字段两边都必须建立索引

```sql
-- user.id主键索引，order.user_id普通索引，联表极快
...
- 原子性 `Atomic`：全部成功 / 全部回滚，不会半完成
- 一致性 `Consistent`：执行前后数据合法（余额不能负数、库存不为负）
- 隔离性 `Isolation`：多事务并发互不干扰，由隔离级别控制
- 持久性 `Durable`：提交后数据永久落盘，宕机不丢失
...
1、悲观锁：默认一定会并发冲突，修改前直接锁住数据，其他人阻塞等待。

- 悲观锁致命坑：索引失效 → 行锁升级为表锁
- 后果：全表阻塞，任何用户更新都等待，线上严重卡顿。

1）共享锁 S（读锁）：`SELECT ... LOCK IN SHARE MODE`

- 多个事务可同时加读锁读数据
- 有读锁时，其他事务不能加写锁更新

2）排他锁 X（写锁）: `SELECT ... FOR UPDATE` 

- 最常用，更新、删除自动加 X 锁
- 加 X 锁后，其他事务不能读、不能写，阻塞等待


2、乐观锁：默认冲突很少，不加锁，不需要事务持续持有锁，性能远高于悲观锁

- 更新时通过版本号 version 校验，版本不一致则放弃更新重试。

**实现原理**

-   查询时带出 version 版本字段
-   更新条件带上 where id=? and version=旧版本
-   更新成功则 version+1；受影响行数 = 0 说明已被其他事务修改，冲突重试
